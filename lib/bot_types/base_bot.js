  ...
  */
  ...
  use(middlewareType, middlewareCallback) {
    if (middlewareType === 'incoming') {
      ...
      this.outgoingMiddleware.use(function(bot, update, message, next) {
  ...
  __runIncomingMiddleware(preMiddlewareUpdate) {
    return new Promise((resolve, reject) => {
      this.incomingMiddleware.run(new BotContext(this, preMiddlewareUpdate), preMiddlewareUpdate, (err, bot, udpate) => {
        if (err) {
  ...
  __runOutgoingMiddleware(preMiddlewareMessage, sendOptions = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.outgoingMiddleware.run(this, sendOptions.update, preMiddlewareMessage, (err, bot, update, message) => {
        if (err) {
