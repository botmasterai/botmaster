...
class BaseBot extends EventEmitter {
  ...
  */
  ...
  /**
   * sendMessageV1() falls back to the sendMessage implementation of whatever
  ...
  sendMessageTo(update, message) {
    
  }
  /**
   * sendMessageToV1() Just makes it easier to send a message without as much
   ...
   * @return {Promise} promise
   */
  sendMessageToV1(message, recipientId) {
  ...
  sendTextMessageTo(update, text) {
    if (typeof update == Object)
      this.reply(update, text)
    else {
      this.sendMessageToV1(text, recipientId);
    }

  }
  /**
   * sendTextMessageToV1() Just makes it easier to send a text message with
   * minimal structure.
   * @param {string} text
   * @param {string} recipientId
   *
   * @return {Promise} promise
   */
  sendTextMessageToV1(text, recipientId) {
    const message = {
      text,
    };
  ...
  use(middlewareType, middlewareCallback) {
    if (middlewareType === 'incoming') {
      ...
      this.outgoingMiddleware.use((err, bot, update, message, next) => {
        if (middlewareCallback.length <= 3) {
          middlewareCallback(err, bot, message, next);
        } else {
          middlewareCallback(err, bot, update, message, next);
        }
      });
    }
  ...
  __runOutgoingMiddleware(preMiddlewareMessage, sendOptions) {
    return new Promise((resolve, reject) => {
      ...
      this.outgoingMiddleware.run(this, preMiddlewareMessage, (err, bot, update, message) => {
        if (err) {
