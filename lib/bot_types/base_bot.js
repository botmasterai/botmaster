...
class BaseBot extends EventEmitter {
  ...
  /**
   * bindUpdate is used to create a new bot instance that on sendMessage sends the update as an sendOption
   * @param {object} update - update to bind to bot
   * @returns {object} bot
   */
  bindUpdate(update) {
  ...
  */
  ...
  __runIncomingMiddleware(preMiddlewareUpdate) {
    return new Promise((resolve, reject) => {
      this.incomingMiddleware.run(
          this.bindUpdate(preMiddlewareUpdate), // use the bot bound with update
          preMiddlewareUpdate, // the update
          (err, bot, udpate) => { // the callback
            if (err) {
  ...
  __runOutgoingMiddleware(preMiddlewareMessage, sendOptions) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.outgoingMiddleware.run(
        this.bindUpdate(sendOptions.update), // use the bot bound with update
        sendOptions.update, // the update
        preMiddlewareMessage, // the message
        (err, bot, update, message) => { // the callback
          if (err) {
