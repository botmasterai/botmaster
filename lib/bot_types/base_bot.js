  ...
  */
  ...
  __runOutgoingMiddleware(preMiddlewareMessage, sendOptions) {
    return new Promise((resolve, reject) => {
      ...
      const middlewareCallback = function middlewareCallback(err, bot, message) {
        if (err) {
      ...
      if (sendOptions && sendOptions.middlewareAccessible) {
        this.outgoingMiddleware.run(this, preMiddlewareMessage,
          sendOptions.middlewareAccessible, middlewareCallback);
      } else {
        this.outgoingMiddleware.run(this, preMiddlewareMessage, middlewareCallback);
      }
    });
  }
