...
class SlackBot extends BaseBot {
  ...
  __applySettings(settings) {
    ...
    if (this.storeTeamInfoInFile) {
      ...
      this.storeTeamInfoHooks.storeTeamInfo = function storeTeamInfo(teamInfo) {
      ...
      this.storeTeamInfoHooks.getTeamInfo = function getTeamInfo(teamId) {
        return new Promise((resolve, reject) => {
  ...
  __authorizeApplicationForTeam(req, res) {
    if (req.query.code) {
      ...
      .then(this.storeTeamInfoHooks.storeTeamInfo.bind(this))
      .then((teamInfo) => {
        this.emit('slackTeamJoined', teamInfo);
        return res.redirect(this.landingPageURL || 'https://my.slack.com');
      })
      .catch((err) => {
        this.emit('error', err);
  ...
  __formatOutgoingMessage(message) {
    ...
    return this.storeTeamInfoHooks.getTeamInfo(teamId)
