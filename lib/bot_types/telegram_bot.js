...
class TelegramBot extends BaseBot {
  ...
  __getAttachments(rawUpdate) {
    ...
    if (shouldReturn()) {
      return new Promise(resolve => resolve(attachments));
    }
  ...
  __makeURL(message) {
    // TODO: adapt when attachment sending is added
    return `${this.baseURL}/sendMessage`;
  }
  ...
  __formatOutgoingMessage(message) {
    ...
    if (message.message.quick_replies) {
      const buttonNames = _.map(message.message.quick_replies, (quickReply) => {
