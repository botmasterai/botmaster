...
class Botmaster extends EventEmitter {
  ...
  addBot(bot) {
    if (bot.requiresWebhook) {
      var monthPath = this.settings && this.settings.mountPath ? this.settings.mountPath :  '/';
      var path = `${monthPath}${bot.type}/${bot.webhookEndpoint}`;
  ...
  removeBot(bot) {
    if (bot.requiresWebhook) {
      var mountPath = this.settings && this.settings.mountPath ? this.settings.mountPath :  '/';
      const path = `${mountPath}${bot.type}/${bot.webhookEndpoint}`;
