...
class Botmaster extends EventEmitter {
  ...
  addBot(bot) {
    ...
    this.bots.push(bot);
    var botWithUpdate = Object.assign({}, bot, {update: update});
    bot.on('update', update => this.emit('update', botWithUpdate, update));
    ...
    this.middlewares.forEach( args => {
      bot.use(args.middlewareType, args.options, args.middlewareCallback)
    })
  ...
  /**
   * Set up middleware for all bots.
   *
  ...
  use(middlewareType, options, middlewareCallback) {
    ...
    this.middlewares.push({middlewareType: middlewareType, options: options, middlewareCallback: middlewareCallback});
