...
class Botmaster extends EventEmitter {
  ...
  use(...args) {
    ...
    const middlewareType = args[0];
    let botType;
    let middlewareCallback;
    if (args.length === 2) {
      middlewareCallback = args[1];
    } else {
      if (typeof args[1] !== 'object') {
        throw new Error('ERROR: invalid coptions. Options should be passed as an object');
      }
      botType = args[1].type;
      middlewareCallback = args[2];
    }

    if ((typeof middlewareType !== 'string' && !(botType instanceof String)) ||
