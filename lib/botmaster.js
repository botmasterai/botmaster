...
class Botmaster extends EventEmitter {
  ...
  constructor(settings = {}) {
    super();
    settings.mountPath = settings.mountPath || '/';
  ...
  addBot(bot) {
    if (bot.requiresWebhook) {
      var path = `${this.settings.mountPath}${bot.type}/${bot.webhookEndpoint}`;
  ...
  removeBot(bot) {
    if (bot.requiresWebhook) {
      const path = `${this.settings.mountPath}${bot.type}/${bot.webhookEndpoint}`;
