...
class Middleware {
  ...
  __runMiddlewareStack(context) {
    ...
    middlewarePromiseStack.then((resolvedValue) => {
      throwThrowableResolvedValue(resolvedValue);
  ...
  __shouldRun(middleware, context) {
    ...
    const shouldRunReceivedEchoUpdate = middleware.includeEcho &&
      get(context.update, 'message.is_echo');
    const shouldRunReceivedDeliveryUpdate = middleware.includeRead &&
      get(context.update, 'read');
    const shouldRunReceivedReadUpdate = middleware.includeRead &&
      get(context.update, 'read');

    if (!shouldRunReceivedEchoUpdate ||
        !shouldRunReceivedDeliveryUpdate ||
        !shouldRunReceivedReadUpdate
        ) {
      return false;
    }
