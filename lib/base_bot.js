...
class BaseBot extends EventEmitter {
  ...
  */
  ...
  // TODO add buildMessage type functions that are analogous to the sendMessage
  // ones. Maybe at this point even rename the sendMessage ones to something
  // I prefer. The sendMessage type functions will then call these build/create
  // functions before using `sendMessage`
  ...
  sendMessage(message) {
    ...
    return botmasterMiddleware.__runOutgoingMiddleware(message, sendOptions)
   ...
   * incoming middleware based on the passed in update
   *
   * @param {object} update
   */
  __emitUpdate(update) {
    return botmasterMiddleware.__runIncomingMiddleware(update)
