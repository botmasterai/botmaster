...
class BaseBot extends EventEmitter {
  ...
  */
  ...
  sendMessage(message) {
    ...
    const outgoingMiddlewarePromise = !sendOptions.ignoreMiddleware
      ? this.middleware.__runOutgoingMiddleware(outgoingMessage, sendOptions.__update)
      : Promise.resolve(message); // don't actually go through middleware

    const responseBody = {};
    return outgoingMiddlewarePromise

    .then((middlewaredMessage) => {
      responseBody.sentOutgoingMessage = middlewaredMessage;
      return this.__formatOutgoingMessage(middlewaredMessage);
    })
    .then((rawMessage) => {
  ...
  __getSendExtraArgs() {
    ...
    if (arguments[0]) {
      if (typeof arguments[0] === 'function') {
        ...
        throw new Error('Last optional argument to sendMessage type function should be' +
    ...
    if (arguments[1]) {
      if (typeof arguments[1] === 'function') {
        ...
        throw new Error('Callback must be of type function when using' +
  ...
  /**
   * This method is used in all helper functions to throw an error if the botType
   * does not support sending messages of a certain type. I.e. you are trying
   * to use bot.sendDefaultButtonMessageTo, but the bot class you are using
   * does not support `sends.quickReply`.
   * @ignore
   */
   ...
   */
  __emitUpdate(update) {
    return this.middleware.__runIncomingMiddleware(update)
  ...
  getUserInfo(userId, cb) {
    if (!this.retrievableUserInfo) {
      throw new TypeError(`Bots of type ${this.type} don't provide access to user info.`)
    }
  ...
  __createBotPatchedWithUpdate(update) {
    ...
    newBot.sendMessage = (message, sendOptions, cb) => {
      if (!sendOptions) {
        ...
        throw new Error(
          `sendOptions should be of type object, not ${typeof sendOptions}`);
      }
  ...
  /**
   * Sugar for listing `bot.middleware.useWrapped`.
