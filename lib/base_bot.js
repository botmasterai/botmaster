  ...
  */
  ...
  sendMessage(message, sendOptions) {
    ...
    .then((middlewaredMessage) => {
      ...
      return this.__formatOutgoingMessage(middlewaredMessage, sendOptions);
    })
    .then((rawMessage) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (sendOptions) {
        if (typeof sendOptions === 'function') {
          ...
          err = new TypeError('sendOptions has to be an object.' +
            `Got ${typeof sendOptions} instead`);
        }
      }
  ...
  getUserInfo(userId) {
    if (!this.retrievableUserInfo) {
      throw new TypeError(`Bots of type ${this.type} don't provide` +
                          'access to user info.');
    }
