...
class BaseBot extends EventEmitter {
  ...
  constructor(settings) {
    ...
    this.retrievableUserInfo = false;
  ...
  */
  ...
  sendMessage(message) {
    ...
    return this.middleware.__runOutgoingMiddleware(outgoingMessage, sendOptions)
  ...
  /**
   * Bot class implementation of the sendMessage function. Each Bot classes
    ...
    for (const messageObject of messageArray) {
      promiseCascade = promiseCascade.then((body) => {
        ...
        if (messageObject.raw) {
          return this.sendRaw(messageObject.raw);
